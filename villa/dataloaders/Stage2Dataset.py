...
class Stage2Dataset(Dataset):
    ...
    def get_text_embs(self):
        ...
        text = list(set(text))
        sent_to_emb = self.get_medclip_text_embs(text)
    ...
    def get_medclip_text_embs(self, sents: list):
        """
        Use CLIP to generate text embeddings.
        ...
        text = tokenizer(
            sents, padding=True, truncation=True, return_tensors="pt", max_length=512
        ).to("cuda")
        with torch.no_grad():
            text_features = model(**text)["last_hidden_state"].detach().cpu()
            text_features /= text_features.norm(dim=-1, keepdim=True)
        text_to_emb = dict(zip(sents, text_features.tolist()))
