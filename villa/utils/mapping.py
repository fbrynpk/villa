...
def compute_mapping_performance(dataset, split):
    """
    Evaluate region-attribute mappings, focusing only on the last region.
    ...
    for idx, row in dataset.iterrows():
        # Focus on the last region (index -1)
        r = row["num_regions"] - 1
        assigned_attr = row["assigned_attributes"][r]
        true_attr = row["reg_to_attr"][r]

        # Compute recall and precision for the last region only
        for attr in assigned_attr:
            mapping_recall[0] += attr in true_attr
            mapping_precision[0] += attr in true_attr
        mapping_precision[1] += len(assigned_attr)
    ...
    print("Mapping Precision (last region):", np.round(mapping_precision, 2))
    print("Mapping Recall (last region):", np.round(mapping_recall, 2))
    print(
        "Mapping F1 (last region):",
        np.round(
            2
            * mapping_precision
            * mapping_recall
